...
#include "../shader/dx9/blur.h"
#include "../shader/dx11/blur.hpp"
...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = ImRect(bb_frame.Min + SCALE(0, 5), bb_frame.Max - SCALE(0, 5));
...
void render_blur(ImGuiWindow* window, float rounding)
{
#ifdef DX_VERSION_9
    DrawBackgroundBlur(window->DrawList, var->device_dx9);
#endif

#ifdef DX_VERSION_11
    draw_background_blur(window->DrawList, var->swap_chain, var->device_dx11, var->device_context, window->Pos, window->Pos + window->Size, rounding);
#endif

}

void render_window_decorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, bool handle_borders_and_resize_grips, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size, bool shadow, ImU32 shadow_col, float shadow_size, bool blur)
{
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
    ...
    if (blur)
    {
        render_blur(window, window->WindowRounding);
    }
    ...
    else
    {
        ...
        if (shadow)
            window->DrawList->AddShadowRect(window->Pos, window->Pos + window->Size, shadow_col, shadow_size, ImVec2(0, 0), ImDrawFlags_ShadowCutOutShapeBackground, window->WindowRounding);
...
bool c_gui::begin(std::string_view name, bool* p_open, ImGuiWindowFlags flags, bool shadow, ImU32 shadow_col, float shadow_size, bool blur)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Apply scrolling
        bool tooltip_opened = false;
        for (int i = 0; i < g.Windows.Size; i++)
        {
            ImGuiWindow* tooltip_window = g.Windows[i];
            if (tooltip_window->Flags & ImGuiWindowFlags_Tooltip && !(window->Flags & ImGuiWindowFlags_Tooltip))
            {
                tooltip_opened = true;
            }
            else
        ...
        if (IsKeyDown(ImGuiKey_MouseWheelY) && (window->Scroll.y == 0.0f || (window->Scroll.y > window->ScrollMax.y - SCROLL_INACCURACY && window->Scroll.y < window->ScrollMax.y + SCROLL_INACCURACY)) && g.ActiveId != GetWindowScrollbarID(window, ImGuiAxis_Y) && (IsMouseHoveringRect(GetWindowPos(), GetWindowPos() + GetWindowSize()) && !tooltip_opened) && window->ScrollMax.y != 0.0f)
        ...
        // FIXME: User code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please get in contact if you are affected (github #4493)
        {
            ...
            render_window_decorations(window, title_bar_rect, title_bar_is_highlight, handle_borders_and_resize_grips, resize_grip_count, resize_grip_col, resize_grip_draw_size, shadow, shadow_col, shadow_size, blur);
