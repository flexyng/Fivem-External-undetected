...
bool input_text_ex(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        ImVec4 color = clr->widgets.label_inactive;
        float offset = 0.f;
    };
    ...
    const ImRect rect(ImVec2(total.Min.x, total.GetCenter().y - SCALE(elements->textfield.height) / 2), ImVec2(total.Max.x, total.GetCenter().y + SCALE(elements->textfield.height) / 2));
    const ImRect clickable(ImVec2(rect.Max.x - round(rect.GetWidth() * elements->textfield.multiplier), rect.Min.y), rect.Max);
    ...
    if (is_multiline)
    {
        ...
        if (!ItemAdd(total, id, &clickable, ImGuiItemFlags_Inputable))
        {
            EndGroup();
            return false;
        }
    ...
    gui->push_font(var->font.inter[2]);
    ...
    animstate->color = ImLerp(animstate->color, render_cursor ? clr->widgets.label : clr->widgets.label_inactive, gui->fixed_speed(18.f));

    // Render frame
    if (!is_multiline)
    {
        draw->render_text(window->DrawList, var->font.inter[1], total.Min - SCALE(elements->widgets.label_padding), total.Max - SCALE(elements->widgets.label_padding), draw->get_clr(animstate->color), label, NULL, NULL, ImVec2(0.f, 0.5f));
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : clickable.Min + SCALE(6, 4);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect rect(ImVec2(rect_pos.x + 0.0f, clickable.Min.y + SCALE(4)), ImVec2(rect_pos.x + rect_size.x, clickable.Max.y - SCALE(4)));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, draw->get_clr(clr->accent, 0.3f), SCALE(elements->widgets.rounding));
                }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(clickable.Min + ImVec2(animstate->offset, SCALE(4)), ImVec2(clickable.Min.x + animstate->offset, clickable.Max.y - SCALE(4)), GetColorU32(ImGuiCol_Text));
...
bool c_gui::text_field(std::string_view label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
...
bool color_input_text_ex(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    struct text_filed_state
    {
        ImVec4 color = clr->widgets.label_inactive;
    ...
    const ImRect total = gui->widget_rect();
    const ImRect rect(ImVec2(total.Min.x, total.GetCenter().y - SCALE(elements->textfield.color_height) / 2), ImVec2(total.Max.x, total.GetCenter().y + SCALE(elements->textfield.color_height) / 2));
    const ImRect clickable(rect.Min, rect.Max - SCALE(28, 0));
    ...
    if (!is_multiline)
    {
        ...
        draw->render_text(window->DrawList, var->font.icons[2], ImVec2(clickable.Max.x, rect.Min.y), rect.Max, draw->get_clr(clr->accent), "E", NULL, NULL, ImVec2(0.5f, 0.5f));
    }

    const ImVec4 clip_rect(clickable.Min.x, clickable.Min.y, clickable.Min.x + inner_size.x, clickable.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : clickable.Min + SCALE(10, 10);
    ImVec2 text_size(0.0f, 0.0f);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ImVec2 rect_size = input_text_calc_text_size_w(&g, p, text_selected_end, &p, NULL, true);
                    if (rect_size.x <= 0.0f) rect_size.x = IM_TRUNC(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
                    ImRect rect(ImVec2(rect_pos.x + 0.0f, clickable.Min.y + SCALE(9)), ImVec2(rect_pos.x + rect_size.x, clickable.Max.y - SCALE(9)));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, draw->get_clr(clr->accent, 0.3f), SCALE(elements->widgets.rounding));
                }
        ...
        if (render_cursor)
        {
            ...
            animstate->offset = ImLerp(animstate->offset, cursor_screen_rect.Min.x - clickable.Min.x, gui->fixed_speed(20.f));
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(clickable.Min + ImVec2(animstate->offset, SCALE(9)), ImVec2(clickable.Min.x + animstate->offset, clickable.Max.y - SCALE(9)), GetColorU32(ImGuiCol_Text));
...
bool c_gui::color_text_field(std::string_view label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()

    return color_input_text_ex(label.data(), NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
