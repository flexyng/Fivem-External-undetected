...
namespace Cheat
{
	...
	public:
		...
		uintptr_t GetHandling()
		{
			...
			return FrameWork::Memory::ReadMemory<uintptr_t>(this + 0x960);
		}
		...
		void FixVehicle()
		{
			...
			//FrameWork::Memory::WriteMemory(this + VehicleState, (BYTE)0x17);
		}
		...
		void SetExtras(uint32_t value)
		{
			...
			FrameWork::Memory::WriteMemory(this + VehicleExtras, value);
		}
	...
	public:
		...
		void SetSpeed(float NewSpeed)
		{
			...
			FrameWork::Memory::WriteMemory(this + PlayerSpeed, NewSpeed);
		}
	...
	public:
		...
		void onepunch_on(bool Toggle)
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
			FrameWork::Memory::WriteMemory<float>(playerinfo + 0x0D6C, 5.0f);
		}

		void onepunch_off(bool Toggle)
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
			FrameWork::Memory::WriteMemory<float>(playerinfo + 0x0D6C, 1.0f);
		}

		void superjump(bool Toggle)
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
			//FrameWork::Memory::WriteMemory<uint32_t>(playerinfo + FrameFlag, 1 << 14);
		}

		void beastjump(bool Toggle)
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
		//	FrameWork::Memory::WriteMemory<uint32_t>(playerinfo + FrameFlag, 1 << 28);
		}

		void invisible_on(bool toggle)
		{
			FrameWork::Memory::WriteMemory(this + 0xAC, 0);

		}

		void invisible_off(bool toggle)
		{
			FrameWork::Memory::WriteMemory(this + 0xAC, 255);
		...
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
			//FrameWork::Memory::WriteMemory<uint32_t>(playerinfo + FrameFlag, 1 << 13);
		}
		...
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
			//FrameWork::Memory::WriteMemory<uint32_t>(playerinfo + FrameFlag, 1 << 12);
		}

		void exploammo(bool Toggle)
		{
			uint64_t playerinfo = FrameWork::Memory::ReadMemory<uint64_t>(this + PlayerInfo);
			//FrameWork::Memory::WriteMemory<uint32_t>(playerinfo + FrameFlag, 1 << 11);
		}

		void infiniteammo(bool Toggle)
		{
			uint64_t weapon_manager = FrameWork::Memory::ReadMemory<uint64_t>(this + WeaponManager);
			uint64_t weapon_info = FrameWork::Memory::ReadMemory<uint64_t>(weapon_manager + 0x20);
			uint64_t ammo_info = FrameWork::Memory::ReadMemory<uint64_t>(weapon_info + 0x60);
			uint64_t ammo_count = FrameWork::Memory::ReadMemory<uint64_t>(ammo_info + 0x8);
			uint64_t ammo_count2 = FrameWork::Memory::ReadMemory<uint64_t>(ammo_count + 0x0);
			FrameWork::Memory::WriteMemory<int>(ammo_count2 + 0x18, 30);
			
		}



		void doubleshoot(bool Toggle)
		{
			uint64_t weapon_manager = FrameWork::Memory::ReadMemory<uint64_t>(this + WeaponManager);
			uint64_t weapon_info = FrameWork::Memory::ReadMemory<uint64_t>(weapon_manager + 0x20);
			FrameWork::Memory::WriteMemory<uint32_t>(weapon_info + 0x013C, 0);
		}


		void vehgodmode_on(bool Toggle)
		{
			uint64_t cur_vehicle = FrameWork::Memory::ReadMemory<uint64_t>(this + LastVehicle);
			FrameWork::Memory::WriteMemory(cur_vehicle + 0x189, true);
		}
		void vehgodmode_off(bool Toggle)
		{
			uint64_t cur_vehicle = FrameWork::Memory::ReadMemory<uint64_t>(this + LastVehicle);
			FrameWork::Memory::WriteMemory(cur_vehicle + 0x189, false);
		}
